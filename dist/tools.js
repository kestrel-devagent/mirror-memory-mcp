...
export const toolDefs = {
    search_history: {
        name: "search_history",
        description: "Search the user's Mirror vault (threads, messages, notes, optional pin boost). " +
    ...
    get_thread: {
        name: "get_thread",
        description: "Fetch a bounded thread payload by id. Prefer composite id provider:conversationId " +
            "(from citation.threadId + citation.provider). Host: GET /api/memory/thread/:id.",
        inputSchema: z.object({
            id: z
                .string()
                .min(1)
                .describe("Thread id — prefer 'chatgpt:…' / 'claude:…' / 'grok:…' composite, or bare id with provider."),
    ...
    list_pins: {
        name: "list_pins",
        description: "List pinned threads as citations {provider, conversationId, title, pinnedAt, citation}. " +
    ...
    write_note: {
        name: "write_note",
        description: "Create a durable shared-memory note other agents can retrieve. " +
    ...
    search_notes: {
        name: "search_notes",
        description: "List/search durable notes only. Host: GET /api/memory/notes?q=&limit=. " +
    ...
    append_memory: {
        name: "append_memory",
        description: "Append to an existing note (POST /api/memory/notes with id) when the host supports it. " +
...
server, client) {
    server.registerTool(toolDefs.search_history.name, {
    ...
    server.registerTool(toolDefs.get_thread.name, {
    ...
    server.registerTool(toolDefs.list_pins.name, {
    ...
    server.registerTool(toolDefs.write_note.name, {
    ...
    server.registerTool(toolDefs.search_notes.name, {
    ...
    server.registerTool(toolDefs.append_memory.name, {
